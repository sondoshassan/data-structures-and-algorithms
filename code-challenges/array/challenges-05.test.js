...
const listFoods = (recipe) => {
  ...
  array.forEach((val) =>{
    ...
    result.push(val.slice(numIndx,val.length));
  })
  return result;
};
...
const splitFoods = (recipe) => {
  ...
  array.forEach(val =>{
    console.log(val);
    let arrayElement = val.split(' ');
    arrayElement.splice(0,2);
    let stringOutput = arrayElement.join(' ');
...
const removeEvenValues = (arr) => {
  // Solution code here...
  let fixArray = [];
  arr.forEach(val => fixArray.push(val));
  fixArray.forEach((val) =>{
    if(val%2 === 0){
...
const removeVowels = (str) => {
  ...
  // let finalString = '';
  let splitArray;
  const vowelArray = ['o','i','e','u','a'];
  vowelArray.forEach(val =>{
    for (let i=0;i<str.length;i++)
      if(val === str[i]){
        splitArray = str.split(val);
        str = splitArray[0] + splitArray[1];
        splitArray = [];
      }}
  )
  return str;
