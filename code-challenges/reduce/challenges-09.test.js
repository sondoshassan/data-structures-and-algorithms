...
const reversedString = (str) => {
  ...
  let final = array.reduce((acc,val)=>{
    acc = val + acc;
    return acc;
  })
  return final;
...
const countNumberOfChildren = (arr) => {
  // Solution code here...
  let array = arr.reduce((acc,val,i) =>{
    if(val.children){
...
const extractChildren = (arr) => {
  ...
  let filterArray = arr.filter(val =>{
    let matches = val.name.match(regEx);
    if(matches){
      return val;
    }
  });
  let array = filterArray.reduce((acc,val,i) =>{
    if(val.children){
      acc = acc +',' +val.children.join(',');
    }
    return acc;
  },'');
  return array.split(',').slice(1,array.length);
};
